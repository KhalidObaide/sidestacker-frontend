...
const Board = forwardRef((props: BoardProps, ref) => {
  ...
  const makeMove = (board: number[][], row: number, direction: Direction, player: Player, updateServer: boolean) => {
      if ( [...ReadyStatuses, GameStatus.Waiting].includes(game.status) ) return;
      if ( game.player === Player.Player1 && game.status !== GameStatus.Player1Turn ) return;
      if ( game.player === Player.Player2 && game.status !== GameStatus.Player2Turn ) return;
      const spot = findSpot(board, row, direction);
      if ( spot === ErrorCase.SpotTaken || spot === ErrorCase.InvalidRow) return

      if ( updateServer ) onMove(row, direction);
      board[spot.row][spot.column] = player
      setMasterBoard(board)
  }
  const runMoves = (input:string) => {
    const moves = input.split('/').slice(0, -1).map(move => {
    ...
    moves.forEach(move => {
        makeMove(board, move.row, move.direction, player, false);
    ...
    <div className={"board " + ( [...ReadyStatuses, GameStatus.Waiting].includes(game.status) ? "disabled" : "")}>
        {masterBoard.map((row, rowIndex) => (
            <div className="row" key={rowIndex}>
                <div onClick={() => makeMove(masterBoard, rowIndex, Direction.Right, game.player, true)}>
                    <Cell cellType={CellType.InsertRight} state={CellState.Empty} />
                </div>
                {row.map((column, columnIndex) => (
                    <Cell cellType={CellType.Cell} state={column} key={columnIndex} />
                ))}
                <div onClick={() => makeMove(masterBoard, rowIndex, Direction.Left, game.player, true)}>
